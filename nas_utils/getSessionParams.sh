...
function sendPacket(){
    ...
    if [ "$set_field_filter" = "$true" ]; then
	case $field in
	    sess_login)
		fieldSelector="User-Name"
	    ;;
	    sess_ip)
		fieldSelector="Framed-IP-Address"
	    ;;
	    sess_services)
		fieldSelector="Cisco-Account-Info"
	    ;;
	esac
	echo $UserName,$coa_header,$AccountStatusQueryCommand | radclient -s $nas_ip:$port coa $secret  | grep $fieldSelector | while IFS='=' read key value; do
        	    echo -n $comma
        	    echo -n $value | tr -d '"'
        	    comma=","
        done
    else
	echo $UserName,$coa_header,$AccountStatusQueryCommand | radclient -s $nas_ip:$port coa $secret 
    fi
